...
use commands::{
    ...
    config::{self, check_username::check_username, verify_password::verify_password},
    init::create_delta_folders::create_folders,
    pull::fetch_user::fetch_user_by_username,
    push::{aws::aws_fn, s3_fn::s3_fn},
};

mod utility;
use serde::{Deserialize, Serialize};
use utility::{
    append_data::append, compress::compress_content_to_file, decompress::decompress_file_lines,
    generate_random_id::generate_random_id, read_file::read_lines,
};
...
fn main() {
            ...
            match decompress_file_lines(&path_string) {
                Ok(lines) => {
                    ...
                    let original_content =
                        fs::read_to_string(&original_path).expect("Failed to read file !!!");
                    let current_path_string =
                        format!("./.delta/current state/{}", &path_string[14..]);
                    ...
                    if !object_path.exists() || !object_path.is_dir() {
                        fs::create_dir(&object_path_string)
    ...
    // pull a repo from server
        ...
        let repo = &args[2];
        let (username, repo_name) = match repo.split_once("-") {
            Some((user, name)) => (user, name),
            None => {
                eprintln!(
                    "Invalid repo format. Expected 'username-repo_name'. Got: {}",
                    repo
                );
                return;
            }
        ...
        let config_path_string = format!("/usr/local/bin/.config");
        let path = Path::new(&config_path_string);
        if !path.exists() || !path.is_file() {
            println!("Firstly authenticate yourself !!!");
            println!("Command -> delta config user.name <username>");
            println!("Command -> delta config user.password <password>");
        ...
        let config_lines = read_lines(&config_path_string);
        ...
        let current_user = &config_lines[0];

        let dot_delta_path = Path::new("./.delta");

        if dot_delta_path.exists() && dot_delta_path.is_dir() {
        } else {
            println!("another delta repository already present !!!");
        }

        if current_user != username {
            let rt = tokio::runtime::Runtime::new().unwrap();

            let user_opt = rt.block_on(fetch_user_by_username(username));
